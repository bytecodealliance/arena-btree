...
pub use super::set_val::SetValZST;

/// An arena for `BTreeSet<T>`.
...
impl<T: Hash> BTreeSet<T> {
    ...
    pub fn hash<H: Hasher>(&self, arena: &SetArena<T>, state: &mut H) {
        self.map.hash(arena, state)
    }
}

impl<T: PartialEq> BTreeSet<T> {
    /// TODO FITZGEN
    pub fn eq(&self, arena: &SetArena<T>, other: &BTreeSet<T>) -> bool {
        self.map.eq(arena, &other.map)
    }

    /// TODO FITZGEN
    pub fn ne(&self, arena: &SetArena<T>, other: &BTreeSet<T>) -> bool {
        self.map.ne(arena, &other.map)
    }
}

impl<T: PartialOrd> BTreeSet<T> {
    /// TODO FITZGEN
    pub fn partial_cmp(&self, arena: &SetArena<T>, other: &BTreeSet<T>) -> Option<Ordering> {
        self.map.partial_cmp(arena, &other.map)
    }
}

impl<T: Ord> BTreeSet<T> {
    /// TODO FITZGEN
    pub fn cmp(&self, arena: &SetArena<T>, other: &BTreeSet<T>) -> Ordering {
        self.map.cmp(arena, &other.map)
    }
}

impl<T: Clone> BTreeSet<T> {
    /// TODO FITZGEN
    pub fn clone(&self, arena: &mut SetArena<T>) -> Self {
        BTreeSet {
            map: self.map.clone(arena),
        }
    }
}
...
pub struct IntoIter<'a, T> {
    iter: super::map::IntoIter<'a, T, SetValZST>,
}
...
impl<T> BTreeSet<T> {
    ...
    {
        Range {
            iter: self.map.range(arena, range),
        }
    }
    ...
    where
        ...
        Q: Ord,
    {
        self.map.contains_key(arena, value)
    }
    ...
    where
        ...
        Q: Ord,
    {
        Recover::get(&self.map, value, arena)
    }
    ...
    {
        self.map
            .insert(arena, value, SetValZST::default())
            .is_none()
    }
    ...
    where
        T: Ord,
    {
        Recover::replace(&mut self.map, value, arena)
    }
    ...
    where
        ...
        Q: Ord,
    {
        self.map.remove(arena, value).is_some()
    }
    ...
    where
        ...
        Q: Ord,
    {
        Recover::take(&mut self.map, value, arena)
    }
    ...
    // /// assert!(a.contains(&5));
    // /// ```
    // pub fn append(&mut self, arena: &mut SetArena<T>, other: &mut Self)
    // where
    //     T: Ord,
    // {
    //     self.map.append(arena, &mut other.map);
    // }
    ...
    where
        ...
        F: 'a + FnMut(&T) -> bool,
    {
        let inner = self.map.drain_filter_inner(arena);
...
impl<T: Ord> BTreeSet<T> {
    /// TODO FITZGEN
    pub fn from_iter<I>(arena: &mut SetArena<T>, iter: I) -> BTreeSet<T>
    where
...
impl<T: Ord> BTreeSet<T> {
    fn from_sorted_iter<I: Iterator<Item = T>>(iter: I, arena: &mut SetArena<T>) -> BTreeSet<T> {
...
impl<T: Ord> BTreeSet<T> {
    /// TODO FITZGEN
    #[inline]
    pub fn extend<Iter>(&mut self, arena: &mut SetArena<T>, iter: Iter)
    where
        Iter: IntoIterator<Item = T>,
    {
        iter.into_iter().for_each(move |elem| {
            self.insert(arena, elem);
        });
    }
}
...
impl<T> Debug for BTreeSet<T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("BTreeSet").finish()
    }
}
